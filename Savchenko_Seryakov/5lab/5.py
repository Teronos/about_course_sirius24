...
#       функции:
#       Переписать на универсальный парсер
#  V    парсер(строка на подстроки)
#  V    ширина как сумма,
#  V    add,
#  V    union, -> упрощение входных интервалов -> проход по всему листу, максимальное объединение так чтобы хранилось максимально необъединяемые
#  V    эквивалентность -> упрощаем по юниону -> Сравнить по длине -> поэлементное сравнение Intervals -
...
#
#     # Перегрузка 'in'
#     # TODO Предусмотреть скобки и провести тесты
#     def __contains__(self, item):
#         match self.left_bracket, self.right_bracket:
#             case TypeOpenBracket.SQUARE.value, TypeClosedBracket.SQUARE.value:
#                 return self.left_border <= item <= self.right_border
#             case TypeOpenBracket.SQUARE.value, TypeClosedBracket.ROUND.value:
#                 return self.left_border <= item < self.right_border
#             case TypeOpenBracket.ROUND.value, TypeClosedBracket.SQUARE.value:
#                 return self.left_border < item <= self.right_border
#             case TypeOpenBracket.ROUND.value, TypeClosedBracket.ROUND.value:
#                 return self.left_border < item < self.right_border
#
#
#
#         # if self.left_border < item < self.right_border:
#         #     return True
#         # else:
#         #     return False
#
...
"""
    # TODO Тест Эквивалентности
    # proms1 = Intervals.parser('[(0, 1], (1, 7], (7, 10]]')
    # # print(proms1.union())
    # proms2 = Intervals.parser('[(0, 5], [5, 10]]')
    # # print(proms2.union())
    # print(proms1.equivalence(proms2))
